<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

<div>
    <div class="officePanelGrid">
        <div></div>
        <div>
            <button type="submit" class="saveBtn"><i class="fa fa-save"></i></button>
            <button type="submit" class="saveBtn"><i class="fa fa-undo"></i></button>
        </div>

        <div class="officePanelColumnLabel" style="text-align: right">Receipt Header Message</div>
        <div></div>
        <div class="officePanelGridLabel">Line 1</div>
        <div><input type="text" class="inputBox" value="Welcome To"></div>
        <div class="officePanelGridLabel">Line 2</div>
        <div><input type="text" class="inputBox" value="NORTH BOWL"></div>
        <div class="officePanelGridLabel">Line 3</div>
        <div><input type="text" class="inputBox" value=""></div>
        <div class="officePanelGridLabel">Line 4</div>
        <div><input type="text" class="inputBox" value="We Appreciate"></div>
        <div class="officePanelGridLabel">Line 5</div>
        <div><input type="text" class="inputBox" value="Your Business!"></div>
        
        <div class="officePanelColumnLabel" style="text-align: right">Receipt Footer Message</div>
        <div></div>
        <div class="officePanelGridLabel">Line 1</div>
        <div><input type="text" class="inputBox" value="Christmas is almost here"></div>
        <div class="officePanelGridLabel">Line 2</div>
        <div><input type="text" class="inputBox" value="Schedule Christmas parties Now!"></div>
        <div class="officePanelGridLabel">Line 3</div>
        <div><input type="text" class="inputBox" value="Inquire At the Desk"></div>
        <div class="officePanelGridLabel">Line 4</div>
        <div><input type="text" class="inputBox" value=""></div>
        <div class="officePanelGridLabel">Line 5</div>
        <div><input type="text" class="inputBox" value=""></div>

        <div class="officePanelColumnLabel" style="text-align: right">Receipt Labels</div>
        <div></div>
        <div class="officePanelGridLabel">Tax Total</div>
        <div><input type="text" class="inputBox" value="Tax Total"></div>
        <div class="officePanelGridLabel">Line Item Total</div>
        <div><input type="text" class="inputBox" value="Total"></div>
        <div class="officePanelGridLabel">Paid</div>
        <div><input type="text" class="inputBox" value="Paid"></div>
        <div class="officePanelGridLabel">Change</div>
        <div><input type="text" class="inputBox" value="Change"></div>
        <div class="officePanelGridLabel">Due</div>
        <div><input type="text" class="inputBox" value="Due"></div>
        <div class="officePanelGridLabel">Payment Total</div>
        <div><input type="text" class="inputBox" value="Payment Total"></div>
        <div class="officePanelGridLabel">Charge Total</div>
        <div><input type="text" class="inputBox" value="Charge Total"></div>
        <div class="officePanelGridLabel">Tender Total</div>
        <div><input type="text" class="inputBox" value="Tender Total"></div>

        <div></div>
        <div>
            <input type="checkbox" id="showpackageitemprices" name="showpackageitemprices" value="showpackageitemprices">
            <label for="showpackageitemprices">Show Package Item Prices</label>
        </div>
    </div>
</div>