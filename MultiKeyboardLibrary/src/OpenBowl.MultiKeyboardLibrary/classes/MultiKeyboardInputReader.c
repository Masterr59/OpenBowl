...
{	
	...
	for (unsigned int i = 0; i < deviceEntryCount; i++)
	{
		...
		{
			char exceptionMessageString[46]; // Theoretical bug: If there are more than 999 devices (4 digits), then sprintf will try to print past exceptionMessageString[45]
	...
	if (keyboardIdentificationDataConstructor == NULL)
	{
...
JNIEXPORT jboolean JNICALL Java_org_openbowl_multikeyboardlibrary_MultiKeyboardInputReader_pollInputs(JNIEnv *environment, jobject thisObject)
{
	...
	for (unsigned int i = 0; i < polledKeyboardListSize; i++)
	{
		...
		{
			char exceptionMessageString[79]; // Theoretical bug: If there are more than 999 devices (4 digits), then sprintf will try to print past exceptionMessageString[78]. It is assumed that errno will generate a maximum of 11 characters (10 digits and 1 negative sign)
			
			if (sprintf(exceptionMessageString, "Could not open element %i of the file descriptor array - errno is %i", deviceEventNumberArray[i], errno) < 0)
			{
	...
	while ((*environment)->GetBooleanField(environment, thisObject, fieldIsInputBeingPolled) == JNI_TRUE)
	{
		...
		{
			char exceptionMessageString[42]; // It is assumed that errno will generate a maximum of 11 characters (10 digits and 1 negative sign)
