					...
					if (deviceData[i + 3] == 'E')
					{
						if (deviceData[i + 4] == 'V')
						{
							if (deviceData[i + 5] == '=')
							{
								...
								int physNullCharacterIndex = 0;
								
								// Figure out where the current phys string ends
								for (int j = 0; j < 1000; j++)
								{
									if (devicePhysArray[keyboardEntryCount][j] == '\0')
									{
										physNullCharacterIndex = j;
										break;
									}
								}
								...
								{	
									char debugKeyboardTypeMessageString[] = " (most likely a keyboard - has EV flags 0x120013 set, and would have been returned by the non-debug library)";
									
									// Append debugKeyboardTypeMessageString to the device phys string
									for (int j = 0; j + physNullCharacterIndex < 1000; j++)
									{
										if (debugKeyboardTypeMessageString[j] == '\0')
										{
											devicePhysArray[keyboardEntryCount][j + physNullCharacterIndex] = '\0';
											break;
										}
										else
										{
											devicePhysArray[keyboardEntryCount][j + physNullCharacterIndex] = debugKeyboardTypeMessageString[j];
										}
									}
								}
								else if ((tempDeviceEventFlags & EV_KEY) == EV_KEY)
								{
									char debugKeyboardTypeMessageString[] = " (might be a keyboard - has EV flag EV_KEY (but not all of 0x120013) set, and would NOT have been returned by the non-debug library)";
								...
								{
									char debugKeyboardTypeMessageString[] = " (most likely NOT a keyboard - has no keyboard-like flags set, and would NOT have been returned by the non-debug library)";
								...
								// Because this is a debug version of the multi-keyboard library, all devices will be returned regardless of their flags
								keyboardEntryCount++;
							}
						}
					}
...
JNIEXPORT jboolean JNICALL Java_org_openbowl_multikeyboardlibrary_MultiKeyboardInputReader_pollInputs(JNIEnv *environment, jobject thisObject)
{
	...
	jfieldID fieldIsNonKeyInputReturned = (*environment)->GetFieldID(environment, multiKeyboardInputReaderClass, "isNonKeyInputReturned", "Z");
	...
	jboolean isNonKeyInputReturned = (*environment)->GetBooleanField(environment, thisObject, fieldIsNonKeyInputReturned);
		...
		for (unsigned int i = 0; i < polledKeyboardListSize; i++)
		{
			...
			if (pollfdArray[i].revents & POLLIN)
			{
				...
				for (unsigned int j = 0; j < bytesRead / inputEventSize; j++)
				{
					...
					// Only return an input event if it is of type EV_KEY (unless the user specifies that they want all events)
					if (currentInputEvent->type == EV_KEY || isNonKeyInputReturned == JNI_TRUE)
					{
